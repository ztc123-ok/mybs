...
r = requests.get('https://travel.qunar.com/p-oi718525-quyuanfenghe', headers=headers)
html_tree = etree.HTML(r.text)
transportation_p = html_tree.xpath("//*[@class='b_detail_info']/div/div[@id='jtzn']/div[2]/*")
travel = ''
for element in transportation_p:
  if (element.find('b') != None):
    travel = travel + ";;" + element.find('b').text + ";;"
  elif (element.find('strong') != None):
    travel = travel + ";;" + element.find('strong').text + ";;"
  else:
    try:
      travel = travel + element.text
    except:
      pass
print(travel)
