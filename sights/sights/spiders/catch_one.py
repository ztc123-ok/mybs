...
class CatchSightsSpider(scrapy.Spider):
    ...
    base_url = "https://you.ctrip.com/sight/Hangzhou14.html?keywords={}"

    search_name = '良渚博物院' # 你可以通过搜索获取匹配度最高的一个景点
    ...
    def __init__(self):
        ...
        if search_name == None:
            print("search_name未传递使用默认参数 '{}',传参请使用--search_name".format(self.search_name))
        else:
            print('init中接收到search_name: ', search_name)
            self.search_name = search_name

    def start_requests(self):
        url = self.base_url.format(self.search_name)
        yield Request(url=url,callback=self.parse)  # 搜索该景点
    ...
    def parse_detail(self, items):
        ...
        # 景点介绍     p标签下需要拼接
        introduction = ''.join(html_tree.xpath(
            "//*[@class='detailModuleRef']//div[@class='LimitHeightText']/div/p/text() | //*[@class='detailModuleRef']//div[@class='LimitHeightText']/div/text()"))
        # 优待政策     div标签下需要拼接
        discount = ''.join(html_tree.xpath(
            "//*[@class='detailModuleRef']/div/div[contains(text(), '优待政策')]/following-sibling::div[1]/div/text()"))
        # 用户昵称
        comments_user = html_tree.xpath(
        ...
        while (flag == 'false' and page1 < 301):
            ...
            # 下一页评论时间
            next_comments_time = html_tree.xpath(
                "//*[@class='commentModuleRef']//div[@class='commentList']/div/div[2]/div[@class='commentFooter']/div[1]/text()")
            # 下一页评论ip
            next_comments_ip = html_tree.xpath(
                "//*[@class='commentModuleRef']//div[@class='commentList']/div/div[2]/div[@class='commentFooter']/div[1]/span/text()[2]")
            # 下一页评论图片
