...
def update(request):
    ...
    if request.method == 'POST':
        sightText = request.POST.get('sightText')
        updateType = request.POST.get('updateType', '0')
        sightId = request.POST.get('sightId', '0')
        ...
        if updateType == '1':
            ...
            print("正在更新一个景点的评论。。。", sightId)
            sightInfo = XcSight.objects.get(id=sightId)
            update_sight.update_sight(sightInfo.url)
        # 标注特定景点
        elif updateType == "2":
            page = int(request.POST.get('page'))
            print("正在标注一个景点评论的情感。。。", sightId)
            use_textCNN.textCNNOne(sightId)
